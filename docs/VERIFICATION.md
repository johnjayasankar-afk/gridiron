...
- **A roof, for a venue reported as indoors.** It cannot be a lid: worked through for the isometric camera at (-70, 94, 106), a deck at y=34 projects its inner edge onto the middle of the field and hides the far end. It is a deck over the stands at y=20 with an opening above the field and a membrane the game shows through. **Held by draw count rather than by the flag**: the roofed scenario draws at least three more than the same game under the sky, which is the deck, its ribs and the membrane. Reading the flag back would have passed with the roof deleted.
