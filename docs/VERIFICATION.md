...
### A claim in the documentation that the code did not make

- **`haze` was defined, unit tested, and documented in two places, and wired to nothing.** The README and the changelog both said the far end of the field is lost in the air in fog. Nothing read the value: the stands had their own fixed distance falloff that has nothing to do with the sky, and the field itself had none at all. This was found by going back through the claims rather than the code, which is the only way this class of thing is ever found.
- It is wired now, and the first version was wrong in a way that had to be looked at rather than reasoned about: fading from 60 units out greyed the half of the field nearest the viewer, because the isometric camera sits about 158 units from the middle and the near touchline is already 110 away. The range now starts past the near edge, and the strength is a fraction of the sky's own figure, because mixing sixty per cent toward white across half a field is a wipe.
- Held by the field probe in two end to end journeys: the snow scenario reports a real amount of haze, and a game the provider reported no weather for reports exactly zero, so a field with no sky is as sharp as it ever was.

### Two more claims the code did not make

Going back through the documentation rather than the code found the haze, and the same pass found two more.

- **"Rain is drawn as a streak" was false.** The rain material had no texture at all, and the code simply scaled `gl_PointSize` by 3.2, with a comment saying "the same point drawn tall". A point sprite is a screen-aligned square and its size is one number: scaling it makes a bigger square and cannot make anything tall. Rain was a larger blob and snow was a square. Both shapes now live in the texture, which is the only place they can: a tall soft gradient for a drop, the existing round one for a flake.
- **The probe proved the wrong thing.** The end to end test asserted the haze through `window.__gridironField`, which reports the number the field computed, not the shader that used it. Breaking the haze patch's fragment anchor on purpose left that test passing.
- **What breaking it actually does was then measured rather than guessed**: the assertion throws to the page, and the field collapses from about thirty four draw calls to seven. The test now watches both, and was re-run with the anchor broken to confirm it fails on the draw count. A check that cannot fail is not a check, and this one could not.

### Making each arena its own

The question asked was whether each stadium could be bespoke to the real arena, and whether modelling them would do it.

- **Modelling them would not.** There are thirty two NFL venues and something like a hundred and thirty in FBS, and no measurements of any of them are available here. A hand built bowl that reads as Lambeau but is not Lambeau is exactly the kind of invention this project refuses everywhere else, and worse, nobody could check it: there would be no way to tell a good model from a wrong one. The bottleneck is not geometry authoring. It is what can be known about a venue and demonstrated.
- **So the provider's venue document was read in full instead**, which turned out to carry more than the two flags already in use. Across a Thursday NFL slate and a college Saturday, **thirty two distinct venues** were fetched: every one reports `grass` and `indoor`, **twenty nine of thirty two** publish at least one photograph, and every one of those twenty nine includes a picture taken inside the bowl. No venue anywhere reports capacity, which was checked on the venue document, the site team endpoint and the season team venue endpoint before giving up on a bowl that scales with how big the real one is.
- **The photograph is the bespoke part**, and it is real rather than modelled. Only addresses the provider lists are used: a guessed path is not the same as being told, and a guessed path here answers with something else, since the same venue id under the college prefix returns a different picture than under the NFL one.
- **Checked against the live provider**: Lambeau Field returns its interior photograph at 2000 by 1125, and Brooks Stadium returns `"images": []`, so the page shows none. That negative was confirmed on the provider's own document rather than assumed from a missing render.
- **Cost.** The provider serves these at full size and ignores `?w=`, so they are one to three megabytes. It sits behind a tab, loads lazily, and holds its space from the reported dimensions, so it costs nothing until somebody opens that tab and never shifts the page when it lands.
- **A roof, for a venue reported as indoors.** It cannot be a lid: worked through for the isometric camera at (-70, 94, 106), a deck at y=34 projects its inner edge onto the middle of the field and hides the far end. It is a deck over the stands at y=20 with an opening above the field and a membrane the game shows through.
- **The first test of that roof was measuring the wrong thing, and it is worth writing down because it passed.** It compared draw calls between the roofed scenario and the same game under the sky, and called the difference the roof. It is not: the two scenarios sit at different plays, so the scenes differ by a ball, a drive and whatever else is on the field at the time. The comparison read three once and fourteen later, and neither number was about a roof. It is the same trap the two bowls were thrown out for earlier in this version, one section further down the page, and it was walked into again anyway. **The bowl now reports whether it drew a roof, and the test reads that.** Run with the roof forced off, the test fails, which is the only thing that makes it a test.
- **What the roof costs was measured separately, on one scene with the roof turned off and on**, which is the isolation the two scenarios could not give: **34 draw calls to 40**. Six for three meshes, because each plane was two sided and every camera this field has sits above the deck, so half of what was drawn could not be seen from anywhere. Turning the faces up and dropping `DoubleSide` took it to **37**: one draw each for the deck, its ribs and the membrane. The independent check came from the performance run, which picks whatever game is first on the slate: it landed on a game at Ford Field, reported `roof: true`, and read **37**.
- **Capacity turned out to be gettable after all, from somewhere else.** The provider has none, which was checked on the venue document, the site team endpoint and the season team venue endpoint. Wikidata has it, and the question was whether it could be matched without ever matching the wrong stadium.
- **Measured before building anything on it.** Across 32 venues from a real slate: **28 matched, 0 ambiguous, 4 unmatched**. The rule that produced that is deliberately strict, and it had to be: "Memorial Stadium" alone returns both Nebraska's, at 87,091, and one in Horfield, England, at 12,100. A match is taken only when the venue is in the United States, the provider's city appears in the entry's administrative chain, and exactly one entry survives both.
- **The full table is 276 venues**, generated from every team's home ground in both leagues, every row carrying the Wikidata id it came from. Coverage where it counts: **28 of the 33 venues hosting games on the day surveyed**. The misses are honest ones, mostly recently renamed grounds. The checked-in numbers were spot checked against the ones anybody would know: Michigan Stadium 107,601, Ohio Stadium 102,780, Lambeau Field 80,750.
- **It is a table, not a lookup.** Nothing queries Wikidata at runtime; the file is generated deliberately and reviewed like any other. A unit test holds that every row cites an entry, that no venue id repeats, and that no name repeats, which is what makes the name fallback exact rather than approximate.
- **That fallback exists because the captured fixtures predate venue ids.** Older scoreboards name their venues and identify them with nothing, so nothing matched at first. Both strings are the provider's own `fullName` for the same place, so comparing them is exact, and it is still only taken while no two venues in the table share a name.
- **Not verifiable:** the real orientation, footprint and roof shape of any venue. The bowl's SIZE is real; its shape is not a claim. The field remains a schematic and says so, and the roof does not pretend to be that venue's roof.

### A final score beside a first quarter clock

Reported from a screenshot, and the clearest kind of bug: two numbers on one screen that cannot both be true.

- Stepping back to play 5 of 158 of a finished game left the scoreboard and the scorebug reading 35 to 14 next to a clock of Q1 13:33. The frame already carried `scoreAfter` for its play; the bug was handed the period and the clock and read the score off a game that had moved on, and the scoreboard was handed no frame at all.
- **Fixed once rather than field by field.** `gameAtFrame` derives the game as it stood at that play, and the scoreboard and the bug both read it, so they cannot drift apart again. Doing it per field would have meant remembering every one of them; this way a caller that forgets gets the whole moment or none of it.
- **Then every other number on the page was checked against the same standard**, because "the score" was the reported symptom rather than the whole question. The situation panel already read the frame, the odds and the drive chart already took one, team totals and leaders already say they cannot rewind, and the play lists are historical by nature. Walked through in the browser at play 1 of 149 of a finished game: board 0-0, bug 0-0, both Q1 15:00, the status pill paused rather than live, and at the first scoring play board and bug both 3-0 at Q1 9:09. **One thing is deliberately left live: the browser tab title.** It is read when the tab is in the background, where the current score is the useful one, and a tab reading Q1 while the game is in overtime would be worse than the mismatch it fixed.
- **The scores roll on the way back through a game, not only forward.** The scoreboard already rolled its digits when a score changed; the bug on the field did not, so scrubbing moved one and jumped the other. The roll is now a hook both of them call. Checked with a mutation observer rather than by eye: jumping to a Cincinnati touchdown put `roll-up` on the Cincinnati number and on nothing else.
- **The scrubber says where the points landed.** One tick per scoring play, in the colour of the team that scored, so a game's shape is on the bar you drag. It is held by a test that counts them against the jump menu's own count and then puts the thumb on three of them and checks the tick is where the thumb stopped, in pixels. The control is drawn rather than left to the platform for exactly this reason: a native range's thumb is whatever width the platform says, and the ticks have to be inset by half of it to line up. Track, thumb and ticks are now placed off one variable. Run with that inset removed, the test fails by 6.7 pixels on the first tick.
- **The scrubber also says where the periods are**, cut out of the bar rather than drawn on it so the notch reads on the filled side and the unfilled side alike. They come from where the period the provider reports actually changes, not from quarters of the bar: in the game measured they fall at 24.3, 51.4 and 73.0 per cent, so quarters of the length would have put all three in the wrong place.
- **The bug says what state the game is in when there is no clock to run.** It read the period and nothing else, so a finished game showed "Q4" beside two final scores and an empty clock, and halftime showed "Q2". It takes the state from `statusShort`, which is where the status pill takes it, so the two cannot disagree: FINAL, FINAL/OT, HALFTIME, END Q3. A play inspected from inside a finished game still gets its clock, because then there is one. Held by a test that checks both halves, and run with the clock branch disabled to confirm the second half fails.
- **The corner under the field is one column now.** The tag, the bug and the orientation caption were three overlays each measuring from the bottom of the field independently; once the bug was between them they met with a pixel to spare and read as one block. The caption moved into the corner's own flex column, which spaces all three the same and puts the whole stack 11 pixels off the bottom.
- **The fix above shipped with a crash in it, and it is the second of its kind in this version.** `shownGame` was derived in a `useMemo` written below the page's early returns, so a render that had no summary yet skipped it and the next one called it: "Rendered more hooks than during the previous render", and the view boundary replaced the entire game page. It passed 112 end-to-end journeys and three browser walkthroughs, because every one of them reached a game by a route that had the summary in hand before the page's chunk mounted. It reproduces on the route nothing tested: a game's own address, opened with the chunk already cached.
- **The comment three lines above the mistake explains the mistake.** It was written earlier in this version, after the field's sound went in below the same returns, and it says exactly what would happen. Knowing the rule did not stop it being broken again, so it is a test now: `tests/hookOrder.test.ts` reads every component in `src` and fails on a hook below a return the component can take. It was validated against both real instances, restored one at a time, and fails on each; it reports nothing on the fixed tree. Eight candidates in the first draft were all false positives from not recognising a file's own lowercase `useX` helpers as new functions, and the check was tightened until it was quiet on everything but the real thing.
- **An end-to-end test for the crash was written, did not detect it, and was deleted.** It navigated to a game's address after warming the chunk and still would not reproduce the render order. A check that passes with the defect restored is worse than no check, so it went, and the static one stands in its place.
- **The 2D field was outside the boundary that exists to catch it.** Only the 3D branch was wrapped, so a failure in the SVG field propagated to the view's boundary and took the game page down. Measured rather than argued: with a throw injected into the 2D field, the page keeps its board, its bug and its rewind with the fix, and with the boundary moved back the test cannot even find the field slot, because the whole view has been replaced.
- **The first version of that test lost a real graphics context, and it destabilised the rest of the file.** Run with it, one of the ball flight or weather journeys failed on each of two runs and a different one each time; run without it the same twenty passed. Losing a context takes the GPU process down for every worker sharing it. It reaches 2D through the effects preference now, which is the same branch and leaves the GPU alone, and the real `webglcontextlost` wiring keeps its own journey in `settings.spec`. Two full runs of the file since: 21 of 21, twice.
- **One stale test was found to be asserting nothing.** The check that the bug paints above the shared canvas read `z-index` off the bug itself, which is `auto`; `Number('auto')` is `NaN`, and `expect(NaN).toBeGreaterThan(-2)` fails rather than passes, so it surfaced as a failure the moment the suite was run in full rather than by name. The number lives on the corner that positions the bug, so the check now walks up to the first ancestor that actually gives it one. Run with that corner pushed to `-5`, it fails.
- **It exposed two more.** The pill announced "Live, Q1 15:00" over a game that ended hours ago, because it was handed a state that said in progress. And the provider's own status text and clock in seconds came along with the play, both describing a moment that has not happened yet from where the viewer is standing.
- **Held by eight unit tests and an end to end journey** that walks a finished game back to its first play and requires the board and the bug to both read 0 to 0, the period to read Q1 and the word Final to be absent, then returns to live and requires the finished score back.
- **The journey was validated against the original bug**: reverting the scoreboard to the live game failed it with `Expected: not "26-14"`, which is exactly the reported symptom.
- **The bug was also sitting on the "Historical view" tag.** Both were anchored to the same corner independently, and the bug's height changes with whether a down was reported, so any fixed nudge would only have moved the collision. They are stacked in one flow now, and the journey asserts the tag ends above the bug begins.

### Building the table from the wrong thing

- The table was assembled from the venues current teams call home. That sounds equivalent to enumerating venues and is not, and running the app against a real slate showed why: **SoFi Stadium was missing entirely**, while two grounds nobody plays in were present. The provider's team documents are stale, placing the Chargers at Dignity Health Sports Park and the Rams at the Los Angeles Memorial Coliseum, homes both teams left years ago.
- It is built from the provider's own venues collection now, which is an enumeration rather than an inference. SoFi comes back at 70,240 against Q19520501.
- **The surface was the bigger find.** It lives only on a venue's own document, never on the scoreboard, so a college Saturday drew mowing stripes on every ground: **0 of 116 cards knew their surface**. Carrying it in the table took that to **116 of 116**, and most of them are synthetic, which is most of a Saturday's cards drawn wrong every time before.
- **The finished table is 958 venues**, 366 of them with a capacity and every one with a surface, from the provider's own enumeration rather than from any roster. Coverage on the two real slates checked after regenerating: 116 of 116 college games and 14 of 14 NFL games know their surface; 83 and 12 know their capacity.
- **A name collision appeared as the table grew.** Two different grounds are called "Greene Stadium". The name fallback was exact while no two venues shared a name, which was true at 276 rows and false past seven hundred. A repeated name is no longer matchable, which a test now asserts by requiring that the repetition exists and that the unique names still cover more than nine tenths of the table.

### A request that did not need making, found by running it

- Spinning the app up against Saturday's slate showed the bowls resolving and the photographs not. The venue's own document was being fetched for the picture, and on a game page that lookup needs a venue id the **summary payload was already carrying**, along with the surface and the photographs themselves.
- So the picture is read straight out of the payload the game page already fetches. A game page now needs no second request at all for any of it, and the venue document is left to answer the scoreboard, whose venue carries an id and a roof and neither of the other two.
- Verified against the live provider on two real games: Michigan Stadium comes back at 107,601 on a synthetic surface with its interior photograph, and Caesars Superdome at 75,167 with its own. Neither made a second request for it.

### The scorebug

- **It was drawn before kickoff**, reading "IOWA - MICH -" beside an empty clock block, which is two dashes and a box on a game that has not started. Found by opening a scheduled game rather than by reasoning about states. It appears once a game does; the field already carries the kickoff time, which is the only question a scheduled game raises.
- Every figure in it is reported or absent, which is checked by rendering it: a down nobody reported draws no row, a timeout count nobody reported draws no pips rather than three, an unreported score is a dash and never a zero, and only one row is ever lit for possession.
- **The red zone had a real gap, found by a test rather than by looking.** It lived in the same row as the down, so a game reported as inside the twenty with no down text showed nothing at all, which is exactly when it matters most. It is drawn on its own now.
- **And it was invisible the first time, twice over.** It used `--bg-1` and `--text-1`, which do not exist in this project's tokens, so those declarations were invalid; and at `z-index: 3` it was drawn underneath the shared canvas. Both were found by inspecting the computed style in a browser rather than by reading the CSS again. An end to end test now requires the bug to sit above the canvas, because below it the whole thing vanishes.

### What a live game finally showed

The Thursday night game kicked off while this was being built, which settled two things that had been listed as unverified since the beginning.

- **The sportsbook line does move during a game.** Read before kickoff and again at 3:38 of the third quarter at Lambeau: the total went from 42.5 at -118 to 43.5 at -105, the moneyline came in from -245 to -238, and the juice on the spread moved from -115 to -112. That is `current` from the core API updating in play, which no amount of reading a finished game could have proven.
- **And it exposed a real gap in the trend.** The handicap never budged off 4.5 all evening. The trend drew the spread and only the spread, so for this game, with genuine movement recorded, it would have drawn nothing and called it a book standing still. It now draws whichever figure moved, the spread first and the total when the spread held. This is the clearest case in the whole version of something only live data could find.
- The recording was made with `scripts/capture-lines.ts` against the live game, and both readings are real.

### Production hardening

- **Memory.** Days nobody looked at were forgotten; the per game state was not. A season's worth of details, delta versions, price histories and recorded lines accumulated for the life of the process. Now a game idle for two hours is dropped with everything held for it, and an engine test proves the count falls from two to one while the game still being polled keeps its task and still serves its whole detail to a newcomer.
- **Latency.** The venue's roof and surface came from a second document that the detail waited on the first time anybody opened a game there. It is fetched in the background now, with an in flight guard so a burst of polls asks once, and unknown is a state the field already draws correctly.
- **A client steering an outbound path.** `nfl-..` parses, and the live odds URL puts the id into two path segments; the URL stays on the provider's host, so this was path steering and not SSRF, and the request would only have 404ed. Every real event id the provider has issued is numeric, checked across 303 of them in the captured scoreboards, so ids that reach a URL path are now required to be digits. Eight refusal cases are tested, including `..`, `../../v2` and an id with a colon in it. The summary endpoint takes its id in a query parameter, where a dot is inert, so it is left alone.
- **The script that writes files** now validates the league and the date it is handed, and refuses a date key from the provider that is not eight digits, because both halves name a file it writes.
- **The security policy was already there and already correct**, which is worth writing down because the first assumption was that it was missing. What was missing was anything keeping its two copies together: it is declared in `server/http.ts` for the Node server and in `vercel.json` for the serverless deployment, and neither can read the other. A test now holds them equal and pins the directives that matter, and an end to end journey requires the browser to refuse nothing across the slate, a game page and the tape.
- **That journey was validated by breaking the policy.** Removing the provider's image host made it fail; restored, it passes. A check that cannot fail is not a check.
- **What was looked at and found sound**: the watch party state is size limited twice, key allowlisted and field by field validated, with no free text anywhere, so there is no injection surface; the static handler resolves and prefix checks every path; origin is checked on state changing routes; and the shipping code carries no TODO, no stray logging, one `@ts-expect-error` for importing plain JavaScript, and no `any`.

### An unreliable suite is itself a production problem

- Across three consecutive full runs, **one heavyweight journey failed each time and a different one each time**: the broadcast camera, the J and K keyboard journey, and the tape's moment link. Every one of them passed on its own, which is the shape of a load problem rather than a defect, but three failures in three runs is not something to wave through.
- **Characterised rather than assumed.** The three specs run together twice: one failure, then none. The suite ran three workers locally and two in CI, and the heavy journeys watch hundreds of frames and assert on timing, so a third browser competing for the machine is enough to move them.
- **At two workers the whole Chrome suite passed**, 100 journeys, 17 skipped by their own guards, none failed. Local now runs what CI runs, which is worth more than the minute it costs.
- This is pre-existing rather than new: the same shape appeared earlier in this version, and two journeys were fixed for it then. What is new is that the cause was measured and the configuration changed to match.

### Measured again, at the end

- Thirteen live fields at 1920x3200 with the replay at 60x, three consecutive solo runs with nothing else on the machine: **zero long tasks in all three**, frames 1.60 to 2.59ms, 152 to 189 draw calls, 24,452 triangles, heap 26 to 41MB. A game page on its own: 0.73 to 0.79ms a frame across 34 draw calls and 4,206 to 4,224 triangles.
- **Measured once more at the end of this pass, solo, after an idle**: thirteen fields at 151 to 189 draw calls, 24,452 triangles, **1.73 to 2.65ms**, no long task over 50ms, heap 42MB. The game page read **37 draw calls, 4,298 triangles, 0.73ms**, and the run now records which game it landed on: Ford Field, `roof: true`. That is the documented 34 plus the roof's three, from a scenario nothing about this pass chose. The snow page read 36 draw calls at 0.55ms, median frame 16.7ms and slowest 17.6 over 240 frames.
- **Nothing in this pass costs a draw call either.** The score ticks, the drawn scrubber, the rolling bug number and the corner column are all HTML and CSS; the field's scene is untouched by them.
- **A bowl the size of the real stadium costs triangles, not draw calls.** The stands are merged into one draw, so a bigger bowl is more of that one draw and not another: the game page is 34 draw calls at every venue measured. The growth is bounded rather than open ended, with the scale clamped at 1.34, held by a unit test, so the largest venue in the table at 107,601 is about a quarter more rows than the reference bowl and the worst case a third.
- **Two bowls were measured side by side and the comparison was thrown out**, because it did not isolate what it claimed to: the larger venue's game was in progress and the smaller one's was final, so the scene contents differed and not only the bowl. It is reported here rather than quietly used, because a measurement that compares two different things is worse than none.
- **Nothing added to this version costs a draw call.** The sky's light and tint ride on materials that were already there, the air on the far end of the field is two instructions in a shader those materials already compile, the floodlights are an opacity on geometry that was already drawn, and the drop and flake shapes are textures on a layer that was already one draw. The game page is the same 34 it was before any of it.
- **A game page in the snow**, which is the one thing on the field that never stops, because everything else is drawn on demand and a still page renders no frames at all: **0.50 to 0.55ms a frame across 36 draw calls**, two more than the same page in the dry. Sixty frames a second held over 240 consecutive frames, median 16.6ms, slowest 18.8ms. It is written to `docs/perf-weather.json` on every run and the suite fails if the slowest frame passes 34ms.
- **The heap is genuinely unsteady between runs**, 24MB once and 40MB twice in these three, and earlier sets read 24 to 26MB with a 41MB outlier. It is reported as a range rather than as whichever number flatters, and it is not a figure this version tried to move.
...
### A pass over the whole thing

Asked for after the crash: make sure it loads quickly and accurately, and find the bugs.

- **Every address now has a journey, opened the way the crash was found**: warmed once, then navigated to directly, and it has to render its own content, log nothing, and name itself in the tab. That is seven routes including the unknown one. It found the tape, which never set a title, so opening it from a game left the tab reading that game's score. Validated by setting the title wrong: the check fails.
- **Four of my first seven route selectors were wrong, and the failures were mine rather than the app's.** Focus was asserted on a chosen game where an untouched focus page has empty slots, and the wall on a hero that only exists once one is picked. Reported here because the first run looked like four broken routes and was four broken assertions.
- **Every game on the card, walked end to end**: thirteen games through the previous-game control, covering final, final in overtime, scheduled and four live fourth quarters. No crash, nothing logged, and each drew what its state calls for: the scheduled game has no scorebug and says no plays are reported, and the overtime final reads FINAL/OT.
- **What each address costs, production build, local server so no network is in it**: the slate 327ms cold and 235ms warm, focus 223/178, the wall 207/130, the tape 475/400, a game 487/477, a team 497/407, with 66 to 74kB transferred. Written to `docs/perf-routes.json`.
- **One of those rows was measuring two different things and was fixed rather than reported.** The tape first read 486ms cold against 2415ms warm, which is backwards. It was: cold found no recording and drew the empty tape, warm found one and drew lanes. The tape is a recorder that accumulates over an afternoon and only persists every twenty seconds, so the two runs were never comparable. It measures the tape's own frame now, like every other row measures its own view.
- **The field is not slow to appear, which is worth writing down because it looked like it was.** Three screenshots in a row caught an empty field and each one looked like a loading bug. Measured: the gap between the scoreboard appearing and the field's first frame is **60ms cold and 60ms warm**, and the slot already holds its camera bar, its corner and its caption while the 3D chunk arrives. The empty screenshots were the browser pane capturing between frames, which is what `frameloop="demand"` means.
- **Dragging the scrubber across a 149 play game holds 60 frames a second.** Median frame **16.6ms**, 95th 17.6, and on a second drag **no frame over 32ms at all**, worst 26.7. The first drag pays a one-off of about 135ms at its third frame, in every run, for whatever the first historical play builds. The test asserts the steady state on the second drag and only the median on the first, because holding warm-up to the price of steady state is holding it to the wrong thing. Written to `docs/perf-scrub.json`.
- **Two smoothness readings were thrown away before those.** The first paced its own loop on `requestAnimationFrame` while measuring `requestAnimationFrame`, so it measured the script. The second was taken in a browser pane that was not on screen, where the browser throttles frames to about one a second: it read a median of 1013ms idle and 993ms scrubbing, which says the page is broken and actually says the pane was hidden. Neither is reported as a result.
- **Mobile at 375 by 812**: no horizontal page scroll, `scrollWidth` equal to the viewport. The corner under the field stays inside it on every side with its 8px gaps intact, and the scrubber keeps its score ticks and period notches.
- **Light theme, and college**: a game page in light reads correctly with the field keeping its own dark world, and a college game carries its rankings and its own ball. Both logos load at 500 by 500 with nothing broken; they, too, were absent from a screenshot taken before they painted, and checked rather than assumed.
- **The replay lab's polling was measured rather than eyeballed**: one request a second to the session's status, about 1kB and 4ms each, which is a clock sync rather than a payload. No other polling runs on a paused replay.


### Making it quicker

Asked for after the pass above: find more bugs, and make it load and run faster without losing anything.

- **Profiled rather than guessed.** A CPU profile of the first drag through a game named the cost: `getProgramInfoLog` and `getShaderInfoLog`, 34ms of the sample. Reading a shader's log makes the processor wait for the card to finish compiling, and three.js does it once per program on first use. It is off in production now and on in development. The field's own shader patches assert their anchors in JavaScript before a shader is ever built, so nothing here depended on the log to know a patch landed.
- **The provider's logos were the largest thing on the page by a long way.** Every team logo is served at 500 by 500 whatever it is drawn at, and the slate drew them at twenty-two to thirty pixels: **1207 kB over the wire, with single logos at 131 kB**. Asked for at the size they are drawn, the slate is **562 kB** and a team page **309 kB** against 1284.
- **The resizing endpoint was checked before anything was built on it.** It returns the exact size asked for, keeps a dark variant distinct from a light one where the provider publishes two, serves `access-control-allow-origin: *` so the ball can still draw a logo into its own texture, and answers for college. Measured across five teams it returned 85 to 98 per cent less. **Above 256 it scales the 500 source up and hands back more than the original**: 164 kB against 94. So 256 is the largest size asked for and anything above it takes the provider's own file. Nine unit tests hold the rewriting, including that it never touches an address that is not one of the provider's team logos and never rewrites one twice.
- **The tape was standing up a graphics context for a view drawn entirely in DOM and SVG**, at 191 kB of three.js. The shared canvas follows the field slots on the page now. The tape went from 453 kB to **259**, a team page from 503 to **309**, and the slate and the game page are unchanged because both are full of fields.
- **Two ways exist to build that canvas into a page that is already on screen, and both are held by a journey**: walking to the slate from the tape, and turning effects from 2D back to Full. Each has to end with a renderer that is drawing, not just a canvas element in the DOM.
- **A development-only failure was chased as though it were a bug, and it was not.** Both of those transitions left every field blank in the dev server: the canvas sat at the full viewport, connected, with no renderer behind it. It reproduces only under React's StrictMode, whose double mount is development behaviour; driven through the real settings control against the production build, both pass. It is written down because two changes were made on the wrong diagnosis before the production build was tried, and one of them cost real performance.
- **That one was caught by measuring.** A `resize={{ scroll: false, debounce: 0 }}` on the canvas, added for the wrong reason and then kept on a plausible-sounding one, took the second drag through a game from no frames over 32ms to **fifteen**, with the 95th frame at 57ms against 25. Removed. A change that cannot be shown to help is not kept because the reasoning sounded good.
- **What the drag reads now**: median **16.6 to 18.6ms** on every run this test has ever taken, and on a quiet machine a worst frame of **28, 28 and 38ms** across three consecutive runs, against the 135ms the test was written against. The first drag no longer pays for shader logs or for decoding a 500 by 500 logo into the ball's skin.
- **The worst frame is recorded and no longer asserted.** After an afternoon of browser work the same code read 33, 33, 102 and 163ms on four runs. That is the machine, and a bound loose enough to survive it is too loose to catch what it was for. The median holds still, so the median is what the test holds.


### A second look for anything left

- **Coming back was costing a first visit.** Measured by `transferSize`, which is zero for anything the browser did not actually fetch: a second visit pulled **480 kB over 28 files**, the same as the first. The hashed files were immutable and fine; the five that are not hashed were on `no-cache` with no `etag` and no `last-modified`, so there was nothing for the browser to revalidate against and each one came back in full. A validator alone took the second visit to four requests and about a kilobyte, all of them 304s; a week on the fonts, icons and favicon took it to **one request and nothing over the wire**. Both were measured separately rather than credited together.
- **The same was wrong on the CDN**, where only `/assets/` had ever been given a policy, so the fonts fell to the default. Four rules added to match.
- **Checked and found already right, so nothing was changed:** the hashed assets are `immutable` and served Brotli; the fonts declare `font-display: swap` and the two that matter are preloaded from the shell; the service worker is network-first for the page and cache-first for hashed files, and a unit test already holds its version to the package's; the replay's polling is one request a second at 0.84 kB and 4ms, and a game page adds none of its own.
- **The steady state was profiled and left alone.** Twelve seconds of a slate with the replay running at thirty times speed: **91.4 per cent idle**, no long task, and of the 1047ms that was busy, **421ms is `getBoundingClientRect`** — the 3D views measuring their own slots every frame, which is how the library places them. It is 3.5 per cent of wall time on a page that is almost entirely idle, and cutting it means fighting the library for something no one can perceive. It is written down rather than acted on, and the DOM churn behind it was measured too: 98 mutations a second at thirty times speed, which is about three a second on a real afternoon.


### A deeper dig, and what it did not find

Asked for a deep audit for anything left. Most of it found the thing already right, which is written down here because a search that found nothing is worth as much as one that did.

- **No leak.** Five minutes of a slate at sixty times speed, with the garbage collector run before each reading: WebGL geometries, textures and programs flat (20 to 23, 29 to 20, 10 to 9), cards flat at 13, views flat at 4. The heap and the DOM grew, 17 to 48MB and 1,700 to 6,200 nodes, and the growth is the moments rail filling toward its cap rather than anything escaping.
- **The rail is capped at 250 and nothing held that.** It is what keeps an afternoon from becoming the whole day, and there was no test on it. Five now: that it stops at the cap over a thousand moments, that it keeps the newest and drops the oldest, that it holds when a burst arrives in one poll rather than one at a time, that the same moment reported twenty times counts once, and that a correction replaces a moment instead of adding one. Validated by raising the cap and by removing it, and both fail.
- **A change was made, measured, and taken out again.** The rail draws all 250 of its moments and about five are on screen, so `content-visibility: auto` looked obviously right. Measured with Chrome's own layout counters while scrolling, it was **worse**: layout time went 43ms to 79ms as the rail filled, where without it the same scroll held 45ms, 43ms and 48ms at 7, 126 and 167 moments. The intersection tracking it adds costs more than laying these items out. The hypothesis behind it was wrong too, and the same numbers say so: **layout does not grow with the rail**.
- **Two measurements were thrown out before those.** Frame gaps were too noisy on this machine to judge a change of that size, and a forced-layout micro-benchmark turned out to be measuring nothing, because setting a custom property nobody reads does not dirty layout.
- **An earlier figure in this document was overstated and is corrected here.** `getBoundingClientRect` was reported at 3.5 per cent of wall time. That reading came from a profile at thirty times speed. At real speed the slate is **98.6 per cent idle**, 163ms busy in twelve seconds, and the rect reads are 23ms of it: **0.2 per cent**. There is nothing there to optimise.
- **A preconnect to the provider's image host, measured and kept anyway.** Four runs each with 150ms of latency emulated: 240ms mean with it and 234ms without, which is no difference. Emulated latency does not model the DNS, TCP and TLS a cold client actually pays, which is the only thing the hint is for, and it costs nothing at runtime. Reported as a null result rather than as a saving.
- **Checked and found already right:** three.js is tree-shaken from 1,423kB of source to 884kB bundled; nothing large is in the main chunk that should not be, the biggest entry after React being 23kB of icons; the provider fetcher already shares identical in-flight requests, which two simultaneous slate requests confirmed by finishing together; and the duplicate requests seen in the dev server are React's StrictMode and do not happen in the production build, which was checked rather than assumed this time.


### Checks

- 634 unit tests, 67 files.
- **147 end-to-end journeys across Chrome and WebKit: 124 run, 23 skipped by their own guards, none failed.** The count was taken from the runner rather than carried forward: the line here read 120 and 104, which had drifted as journeys were added without it being re-counted. Fourteen are the tape and the pulse, fourteen are the field in 3D and the reel, and five are odds and win probability.
