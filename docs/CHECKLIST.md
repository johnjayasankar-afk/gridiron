...
| Requirement | Status | Evidence |
| --- | --- | --- |
| The tape: the whole day on one clock, keyboard readable, with a pulse on every card | Done | E2E `the tape` (14 journeys); Unit `tape.test.ts`; Screens `tape-1440`, `tape-1440-scrub` |
| The sportsbook line moves during a game | Done | Unit `espnOdds.test.ts` (the live line from two captured core documents, and the URL guard); read end to end against the live provider on 24 Sep 2026: GB opened at −7.5 −110 and stood at −4.5 −115 |
| That line rewinds to the play being looked at, in a live session and in a replay | Done | Unit `lineHistory.test.ts` (29, including the rendered panel) and `replayLines.test.ts` (7); `scripts/capture-lines.ts` run against the live provider |
| Every game lit by its own reported sky, with rain and snow where reported | Done | Unit `sky.test.ts` (23); E2E `the sky over the field` (2) and `a game page under weather`; Screens `game-1440-weather`; Browser: real rain at Coastal Carolina and a real night sky at Lambeau, 24 Sep 2026 |
| Grass and synthetic surfaces drawn as themselves | Done | Unit `sky.test.ts` (the 2D field drops its stripes); Browser: Lambeau reports `grass: true`, SoFi and Brooks Stadium report `grass: false` |
| Opt-in field sounds, off everywhere until switched on | Done | Unit `fieldSound.test.ts` (4); E2E `field sounds play on a game page, only when turned on` (counts oscillators in three states) |
| The security policy is the same however the page is served, and blocks nothing | Done | Unit `headers.test.ts` (4); E2E `the policy blocks nothing the app needs`, validated by breaking the policy and watching it fail |
| A long-running server does not grow without bound | Done | Unit `engine.test.ts` (a game idle two hours is dropped with everything held for it; one still polled is not) |
| The field survives a change of style | Done | E2E `the field survives a change of style, both ways` |
| Each arena is bespoke to the real venue, as far as the provider reports it | Done | Unit `venue.test.ts` (10); Browser: Lambeau's interior photograph, its grass and its open air, against the live provider with a game in progress |
| A venue reported as indoors is given a roof | Done | E2E `a venue the provider says has a roof is given one, and one without is not` (asserted by draw count, not by the flag); Screens `game-1440-indoors` |
| Every card knows what its game is played on | Done | 0 of 116 cards on a real college Saturday knew their surface before; 116 of 116 after. Unit `bowl.test.ts` holds the table's integrity |
| The bowl is the size of the real stadium | Done | Unit `bowl.test.ts` (9, including the checked-in table's integrity); 276 venues generated by `scripts/capture-venues.ts`, 28 of 33 venues hosting games that day matched; Screens `game-1440-bowl-big` (MetLife, 82,500) and `game-1440-bowl-small` (Lucas Oil, 62,421) |
| Looking at an earlier play shows that moment, not the latest one | Done | Unit `gameAtFrame.test.ts` (8) and `scoreBug.test.ts`; E2E `the score, the clock and the period are all as of the play being looked at`, validated by reverting the fix and watching it fail; Screens `game-1440-historical` |
| A scorebug on the field, carrying the state of the down | Done | Unit `scoreBug.test.ts` (9); E2E `the scorebug carries the state of the down, on the field`, which also requires it to sit above the shared canvas |
| The sportsbook line moves during a live game | Done | Observed at Lambeau on 24 Sep 2026 at 3:38 of the third quarter: total 42.5 to 43.5, moneyline −245 to −238, spread juice −115 to −112, recorded by `scripts/capture-lines.ts` |

## Not verifiable during the build

- **Timeouts on the scorebug.** The provider sends `homeTimeouts` and `awayTimeouts` on a live scoreboard situation; no captured fixture has them, and no game was in progress when the bug was built. The pips are held by render tests against supplied counts, and draw nothing at all where the provider gave none. What has not been seen is a real timeout being spent.
- **A live game, at last.** The Thursday night game kicked off at Lambeau while this version was being finished, and Gridiron was watched against it in progress: the live sportsbook line moving in play, the reported night sky, the venue's real photograph and a real line recording. What is still unwatched is weather actually changing during a game, and a full Sunday or a college Saturday at scale. Nothing was live while Gridiron was first built (Monday 14 September 2026, before Monday Night Football). The live feed was verified for scheduled and final states and the honest empty state. In-game behavior was verified with captured real games in the replay lab, through the same normalization and engine. Since 0.4 that includes win probability after each play and the closing lines. Win probability updates, line movement during a game, Kalshi prices and Kalshi price history were not observed during a live game; Kalshi prices and history were read only for a scheduled game.
